# Case Studies Ads Thành Công 2026 — INDEX

## Phân Cấp Theo Ngân Sách & Phạm Vi

| Cấp Độ | Ngân Sách/Tháng | Phạm Vi | Số Cases |
|--------|-----------------|---------|----------|
| Level 1: Local/Small | $500-$3K | Địa phương, niche micro-market | 3 cases × 3 scenarios |
| Level 2: Regional/Mid-Market | $3K-$25K | Multiple states/countries | 3 cases × 3 scenarios |
| Level 3: Global/Large | $25K-$100K+ | National/global multi-market | 3 cases × 3 scenarios |

## Danh Sách Cases

### LEVEL 1 — PHẠM VI NHỎ / LOCAL

- `cs-local-service-titan-driveways.md` — Dịch vụ địa phương, CPL -50%
- `cs-small-ecommerce-soda-spoon.md` — E-commerce nhỏ, ROAS 3.61x (1 ad duy nhất)
- `cs-b2b-lead-gen-counselling.md` — Lead gen B2B, CPL $4.98 global

### LEVEL 2 — PHẠM VI TRUNG BÌNH / REGIONAL

- `cs-dtc-caraway-home.md` — DTC cookware, ROAS 2.9x→4.6x, CAC -34%
- `cs-dtc-cuts-clothing.md` — Fashion/Accessories, ROAS 2.9x→4.8x, CPA $48→$34
- `cs-high-ticket-home-improvement.md` — Home improvement, ROAS 1.18→6.47, $700K revenue

### LEVEL 3 — PHẠM VI LỚN / GLOBAL

- `cs-chomps-wellness.md` — DTC supplement, ROAS 3.2x→5.1x, CAC -31%
- `cs-hexclad-cookware.md` — Premium cookware, ROAS 2.4x→3.8x, CPA $88→$61
- `cs-momentous-nutrition.md` — Performance nutrition, ROAS 2.4x→4.1x, CAC $68→$41

### TỔNG HỢP

- `cs-key-insights-2026.md` — 10 bài học chiến lược tổng hợp
- `cs-comparison-matrix.md` — So sánh cross-case benchmarks

## Attribution Sources

| Source | Cases Covered |
|--------|--------------|
| D2C Times | Caraway, Cuts, Chomps, Hexclad, Momentous |
| UM Marketing | Home Improvement, Digital Education |
| Soda Spoon | Small e-commerce single-ad case |
| Digital Sales Ireland | Titan Driveways (local service) |
| Amninder Singh | Mental health counselling lead gen |

*Created: 2026-06-15 | Updated from comprehensive analysis in memory/ads-campaigns-comprehensive-analysis-2026-06.md*
